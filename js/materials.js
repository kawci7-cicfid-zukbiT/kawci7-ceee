...
// ====================================================================
// PRESET LAMINATES — seeded once at first launch.
// Each preset references real material IDs from materials.json.
// Estimated WVTR/OTR values are placeholders; users can press "Load"
// in the Laminates DB to load any preset into the Calculator and get
// the real value computed by the series-resistance engine.
// ====================================================================
var PRESET_LAMINATES = [
    {
        name: 'Coffee pouch — high-barrier metallized',
        mode: 'wvtr',
        temperature: 23, humidity: 50,
        total: 0.08, totalThickness: 102,
        recyclable: false, monoStructure: false, layerCount: 3,
        layers: [
            { mid: 17, thick: 12 },   // PET 12µm
            { mid: 43, thick: 20 },   // Metallyte MM480 (BOPP met)
            { mid: 56, thick: 70 }    // PE Ethy-Lyte HD200
        ]
    },
    {
        name: 'Snack bag — BOPP/MET-BOPP/PE',
        mode: 'wvtr',
        temperature: 23, humidity: 50,
        total: 0.32, totalThickness: 75,
        recyclable: false, monoStructure: false, layerCount: 3,
        layers: [
            { mid: 24, thick: 20 },   // BOPP Bicor 25MB400
            { mid: 43, thick: 20 },   // Metallyte (met BOPP)
            { mid: 56, thick: 35 }    // PE
        ]
    },
    {
        name: 'Cheese pack — PET/EVOH/PE (high O₂ barrier)',
        mode: 'otr',
        temperature: 23, humidity: 50,
        total: 0.45, totalThickness: 90,
        recyclable: false, monoStructure: false, layerCount: 3,
        layers: [
            { mid: 17, thick: 12 },   // PET
            { mid: 3,  thick: 8 },    // EVOH F171B
            { mid: 56, thick: 70 }    // PE
        ]
    },
    {
        name: 'MAP fresh-meat tray lid — PA/EVOH/PE',
        mode: 'otr',
        temperature: 5,  humidity: 80,
        total: 0.85, totalThickness: 105,
        recyclable: false, monoStructure: false, layerCount: 3,
        layers: [
            { mid: 88, thick: 15 },   // FILMON BX (PA)
            { mid: 3,  thick: 10 },   // EVOH
            { mid: 56, thick: 80 }    // PE
        ]
    },
    {
        name: 'Pharma cold-form blister — Aclar laminate',
        mode: 'wvtr',
        temperature: 25, humidity: 60,
        total: 0.05, totalThickness: 90,
        recyclable: false, monoStructure: false, layerCount: 1,
        layers: [
            { mid: 91, thick: 90 }    // Aclar Rx160 (pharma)
        ]
    },
    {
    ...
    {
        name: 'Mono-PE pouch (recyclable)',
        mode: 'wvtr',
        temperature: 23, humidity: 50,
        total: 3.50, totalThickness: 100,
        recyclable: true, monoStructure: true, layerCount: 1,
        layers: [
            { mid: 56, thick: 100 }
        ]
    },
    {
        name: 'Mono-PP recyclable laminate — BOPP/CPP',
        mode: 'wvtr',
        temperature: 23, humidity: 50,
        total: 2.20, totalThickness: 50,
        recyclable: true, monoStructure: true, layerCount: 2,
        layers: [
            { mid: 24, thick: 20 },    // BOPP
            { mid: 151, thick: 30 }    // Cosmo HB Met CPP — same family PP
        ]
    },
    {
        name: 'Metallized PET pouch — PET/MET-PET/PE',
        mode: 'wvtr',
        temperature: 23, humidity: 50,
        total: 0.15, totalThickness: 79,
        recyclable: false, monoStructure: false, layerCount: 3,
        layers: [
            { mid: 17, thick: 12 },    // PET
            { mid: 18, thick: 12 },    // PET metallized
            { mid: 56, thick: 55 }     // PE
        ]
    },
    {
        name: 'Compostable PLA/cellulose pouch',
        mode: 'wvtr',
        temperature: 23, humidity: 50,
        total: 6.80, totalThickness: 65,
        recyclable: false, monoStructure: false, layerCount: 2,
        layers: [
            { mid: 114, thick: 25 },   // Ingeo PLA
            { mid: 191, thick: 40 }    // NatureFlex NKME (metallized cellulose)
        ]
    }
];

function loadPresetLaminates() {
    var KEY = 'wvtr_preset_lams_v1_loaded';

    // Validate that referenced materials exist; skip presets with missing layers
    var matIds = {};
    DB.materials.forEach(function(m) { matIds[String(m.id)] = true; });

    // Count how many presets are currently in DB (by name match)
    var presetNames = PRESET_LAMINATES.map(function(p){ return p.name.trim().toLowerCase(); });
    var existingPresets = DB.laminates.filter(function(l) {
        return presetNames.indexOf((l.name || '').trim().toLowerCase()) >= 0;
    }).length;

    // If the flag is set AND at least one preset is in DB, the user has either
    // them all or deleted some on purpose — don't re-seed.
    if (localStorage.getItem(KEY) && existingPresets > 0) return;

    var added = 0;
    PRESET_LAMINATES.forEach(function(p, idx) {
        var allValid = p.layers.every(function(L) { return matIds[String(L.mid)]; });
        if (!allValid) {
            console.warn('Preset skipped (missing material refs):', p.name);
            return;
        }
        var dupe = DB.laminates.some(function(l) {
            return (l.name || '').trim().toLowerCase() === p.name.trim().toLowerCase();
        });
        if (dupe) return;

        var lam = Object.assign({}, p, {
            id: Date.now() + idx,
            _preset: true
        });
        DB.laminates.push(lam);
        added++;
    });

    if (added > 0) {
        DB.save();
        console.log('🧪 Loaded ' + added + ' preset laminates');
    }
