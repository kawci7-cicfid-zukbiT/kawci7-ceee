...
async function saveCompanyMaterial(mat) {
    ...
    try {
        var payload = {
            ...
            application: mat.application || 'neutral',
...
function renderCompanyMaterialsPage() {
    ...
    return '<div style="padding:0.25rem 0">' +
        ...
        '<div class="form-group" style="margin:0"><label>Application</label>' +
            '<select class="form-input" id="co-mf-app" onchange="coMatApplyFilters()" style="font-size:0.78rem">' +
...
function coMatApplyFilters() {
    ...
    var fapp    = document.getElementById('co-mf-app')?.value || '';
    ...
    var filtered = _companyMats.filter(function(m) {
        ...
        if (fapp) {
            var matApp = (typeof normalizeApplication === 'function') ? normalizeApplication(m.application) : (m.application || 'neutral');
            if (matApp !== fapp) return false;
        }
        if (q) {
...
function _coMatCardHTML(m, q) {
    ...
    // Application badge — same colour-coded scheme used in Community DB
    var appBadge = '';
    if (typeof getApplicationMeta === 'function') {
        var appMeta = getApplicationMeta(m.application);
        appBadge = '<span class="badge" style="font-size:0.6rem;background:'+appMeta.bg+';color:'+appMeta.color+';border:1px solid '+appMeta.color+'33;white-space:nowrap">'+appMeta.icon+' '+appMeta.label+'</span> ';
    }
...
function _showCompanyMatModal(mat) {
    ...
    var body =
        ...
        '<div class="form-group"><label>Application Domain</label>' +
            '<select class="form-input" id="co-mat-application">' +
            (typeof APPLICATION_DOMAINS !== 'undefined'
                ? APPLICATION_DOMAINS.map(function(a){
                    var sel = (typeof normalizeApplication === 'function'
                        ? normalizeApplication(mat?mat.application:'neutral')
                        : (mat?mat.application:'neutral')) === a.key ? ' selected' : '';
    ...
    Modal.open(mat ? 'Edit Company Material' : 'Add Company Material', body, function() {
        ...
        var application  = document.getElementById('co-mat-application')?.value || 'neutral';
        ...
        var matData = {
            ...
            application: (typeof normalizeApplication === 'function') ? normalizeApplication(application) : application,
...
// Filters/expanded state for company laminates (persisted in session)
if (typeof window !== 'undefined' && !window._coLamFilters) {
    window._coLamFilters = { search: '', recyclable: '', layerCount: '', application: '', sort: 'name' };
}
if (typeof window !== 'undefined' && !window._coLamExpanded) window._coLamExpanded = {};

function coLamDominantApplication(lam) {
...
function coLamUpdateFilter(field, value) {
    window._coLamFilters[field] = value;
    _renderCompanyLamList();
}

function coLamToggleExpanded(id) {
    window._coLamExpanded[id] = !window._coLamExpanded[id];
    _renderCompanyLamList();
}

function coLamClearFilters() {
    window._coLamFilters = { search: '', recyclable: '', layerCount: '', application: '', sort: 'name' };
    _renderCompanyLamList();
}

function _coLamLayerListHTML(lam) {
    ...
    for (var i = 0; i < lam.layers.length; i++) {
        ...
        var bg = mat ? '#f8fafc' : '#fef2f2';
        html += '<div style="display:flex;align-items:center;gap:8px;padding:5px 9px;background:' + bg + ';border-radius:6px;font-size:.75rem">' +
            '<div style="font-weight:700;color:var(--text-light);min-width:18px;font-size:.7rem">' + (i + 1) + '</div>' +
            dot +
            '<div style="flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="' + nm + '">' + nm + (fam ? ' <span style="color:var(--text-light);font-size:.66rem">· ' + fam + '</span>' : '') + '</div>' +
...
function _renderCompanyLamList() {
    ...
    var filters = window._coLamFilters;

    var pool = _companyLams.filter(function(l){ return l.mode === State.mode; });

    if (!_companyLams.length) {
        listEl.innerHTML = '<div class="card" style="text-align:center;padding:3rem 1.5rem">' +
            ...
            '</div>' +
            '<h3 style="font-size:1rem;font-weight:700;color:#0f172a;margin:0 0 .5rem">No laminates shared yet</h3>' +
            '<p style="font-size:.82rem;color:var(--text-light);margin:0 0 1.25rem;max-width:380px;margin-left:auto;margin-right:auto;line-height:1.6">Build a structure in the Calculator and share it to your company.</p>' +
            '</div>';
        return;
    }
    ...
    var filtered = pool.filter(function(l) {
        ...
        if (filters.application) {
            if (coLamDominantApplication(l) !== filters.application) return false;
        }
        if (q) {
            var hay = (l.name || '').toLowerCase();
            if (l.layers) {
                for (var i = 0; i < l.layers.length; i++) {
                    var mat = DB.materials.find(function(m){ return String(m.id) === String(l.layers[i].mid); });
    ...
    else if (filters.sort === 'perf')    filtered.sort(function(a,b){ return (a.total||0) - (b.total||0); });
    else if (filters.sort === 'thick')   filtered.sort(function(a,b){ return (a.totalThickness||0) - (b.totalThickness||0); });
    else if (filters.sort === 'layers')  filtered.sort(function(a,b){ return ((a.layers&&a.layers.length)||0) - ((b.layers&&b.layers.length)||0); });

    var html = '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.6rem;flex-wrap:wrap;gap:8px">' +
        '<div style="font-size:0.85rem;color:var(--text-light)">Showing <strong style="color:#0f172a">' + filtered.length + '</strong> of ' + pool.length + ' · ' + modeUpper + ' mode</div>' +
        '</div>';
    ...
    html += '<div class="card" style="margin-bottom:0.75rem">' +
        ...
        '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:8px;margin-bottom:0.6rem">' +
            '<div class="form-group" style="margin:0"><label>Recyclable</label>' +
                '<select class="form-input" onchange="coLamUpdateFilter(\'recyclable\',this.value)" style="font-size:0.78rem">' +
            ...
            '<div class="form-group" style="margin:0"><label>Layers</label>' +
                '<select class="form-input" onchange="coLamUpdateFilter(\'layerCount\',this.value)" style="font-size:0.78rem">' +
                ...
                '</select></div>' +
            '<div class="form-group" style="margin:0"><label>Application</label>' +
                '<select class="form-input" onchange="coLamUpdateFilter(\'application\',this.value)" style="font-size:0.78rem">' +
            ...
            '<div class="form-group" style="margin:0"><label>Sort by</label>' +
                '<select class="form-input" onchange="coLamUpdateFilter(\'sort\',this.value)" style="font-size:0.78rem">' +
        ...
        '<div style="position:relative">' +
            '<input type="text" class="form-input" style="font-size:0.82rem"' +
            ' placeholder="Search by name or contained material…"' +
            ' value="' + (filters.search || '') + '"' +
            ' oninput="window._coLamFilters.search=this.value;clearTimeout(window._coLamSearchT);window._coLamSearchT=setTimeout(_renderCompanyLamList,220)">' +
        '</div>' +
        ((filters.recyclable || filters.layerCount || filters.application || filters.search)
            ? '<div style="margin-top:.5rem"><button class="btn btn-sm btn-outline" onclick="coLamClearFilters()" style="font-size:.72rem">× Clear all filters</button></div>'
            : '') +
        '</div>';

    if (!filtered.length) {
        html += '<div class="card" style="text-align:center;padding:2rem 1rem"><p style="color:var(--text-light);margin:0">No laminates match these filters.</p></div>';
    ...
    for (var idx = 0; idx < filtered.length; idx++) {
        ...
        var expanded = !!window._coLamExpanded[l._companyLamId || l.id];
        var nLayers = (l.layers && l.layers.length) || l.layerCount || 0;
        var dominant = coLamDominantApplication(l);
        ...
        var totalVal = l.total || 0;
        if (totalVal && totalVal < 0.1)      { perfClass = 'ultra'; perfLabel = 'Ultra-high barrier'; }
        else if (totalVal && totalVal < 1)   { perfClass = 'high';  perfLabel = 'High barrier'; }
        else if (totalVal && totalVal < 10)  { perfClass = 'med';   perfLabel = 'Medium barrier'; }

        var perfColors = { ultra:'#7c3aed', high:'#2563eb', med:'#0891b2', low:'#64748b' };
        var perfBg     = { ultra:'#ede9fe', high:'#eff6ff', med:'#ecfeff', low:'#f1f5f9' };

        var idStr = String(l._companyLamId || l.id);
            ...
            '<div style="padding:.9rem 1rem">' +
                '<div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;margin-bottom:.6rem">' +
                    '<div style="flex:1;min-width:0">' +
                        '<div style="font-weight:700;font-size:.92rem;color:#0f172a;line-height:1.3;overflow:hidden;text-overflow:ellipsis;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical" title="' + l.name + '">' + l.name + '</div>' +
                    '</div>' +
                ...
                '<div style="display:flex;align-items:flex-end;gap:14px;padding:.6rem .75rem;background:' + perfBg[perfClass] + ';border-radius:8px;margin-bottom:.55rem">' +
                    '<div style="flex:1">' +
                        ...
                        '<div style="font-size:1.4rem;font-weight:800;color:' + perfColors[perfClass] + ';line-height:1.1;font-variant-numeric:tabular-nums">' + (totalVal ? totalVal.toFixed(totalVal < 0.01 ? 5 : (totalVal < 1 ? 4 : 3)) : '—') + '</div>' +
                ...
                (nLayers > 0 ?
                    '<div style="border-top:1px solid var(--border);margin-top:.4rem;padding-top:.55rem">' +
                        '<button onclick="coLamToggleExpanded(\'' + idStr + '\')" style="background:none;border:none;padding:0;color:var(--primary);font-size:.72rem;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:4px;margin-bottom:' + (expanded ? '.5rem' : '0') + '">' +
                        ...
                        '</button>' +
                        (expanded ? _coLamLayerListHTML(l) : '') +
                    '</div>' : '') +
                '<div style="display:flex;justify-content:flex-end;gap:6px;margin-top:.6rem;padding-top:.5rem;border-top:1px solid var(--border)">' +
                    '<button class="btn btn-sm btn-outline" onclick="loadCompanyLaminateInCalc(\'' + String(l.id) + '\')" title="Load this laminate into the Calculator">Load</button>' +
