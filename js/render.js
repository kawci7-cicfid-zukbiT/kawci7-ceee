// ====================================================================
// js/render.js
// All render functions, post-render handlers, calc handlers,
// condition/layer UI, compare, laminates, sensitivity, arrhenius UI
// ====================================================================
...
function searchMaterialWeb(matName) {
  ...
  Modal.open('Search Online: ' + matName,
    ...
    '</div></div>' +
    '<div class="alert alert-info"><svg class="alert-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg><div><strong>When you find the data:</strong><br>1. Copy WVTR, OTR, temperature and humidity<br>2. Return to this app<br>3. Click "Add Material"<br>4. Paste the values<br><br><strong>Useful sites:</strong><br>MatWeb.com, Omnexus.specialchem.com, Plasticseurope.org, ULprospector.com</div></div>',
  ...
  <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:1px;background:#e2e8f0;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden;margin-bottom:2rem">
    <div style="background:#fff;padding:1.75rem 1.5rem">
      ...
      <div style="margin-top:1.25rem;padding-top:1rem;border-top:1px solid #f1f5f9">
        <button onclick="document.getElementById('nav-tabs').querySelector('[data-tab=materials]').click()" style="font-size:0.78rem;color:#2563eb;background:none;border:none;cursor:pointer;padding:0;font-weight:600">Explore database →</button>
      </div>
    </div>
    <div style="background:#fff;padding:1.75rem 1.5rem">
      ...
      <div style="margin-top:1.25rem;padding-top:1rem;border-top:1px solid #f1f5f9">
        <button onclick="document.getElementById('nav-tabs').querySelector('[data-tab=arrhenius]').click()" style="font-size:0.78rem;color:#2563eb;background:none;border:none;cursor:pointer;padding:0;font-weight:600">Run analysis →</button>
      </div>
    </div>
    <div style="background:#fff;padding:1.75rem 1.5rem">
      ...
      <div style="margin-top:1.25rem;padding-top:1rem;border-top:1px solid #f1f5f9">
        <button onclick="document.getElementById('nav-tabs').querySelector('[data-tab=laminates]').click()" style="font-size:0.78rem;color:#2563eb;background:none;border:none;cursor:pointer;padding:0;font-weight:600">View library →</button>
      </div>
    </div>
  </div>
  ...
  <div style="padding:1.5rem 0;border-top:1px solid #e2e8f0;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:0.75rem">
    ...
    <div style="display:flex;align-items:center;gap:1rem;flex-wrap:wrap">
      <a href="mailto:wvtrotrcalculator@gmail.com?subject=Feedback%20-%20WVTR%2FOTR%20Calculator"
        ...
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:12px;height:12px"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,12 2,6"/></svg>
        Send Feedback
      </a>
    </div>
  </div>
...
PET (12µm) + LDPE (50µm):<br>
<em>R<sub>PET</sub> = 12 / (30.0 × 12) = 0.0333</em><br>
<em>R<sub>LDPE</sub> = 50 / (4.0 × 25) = 0.5000</em><br>
<em>R<sub>total</sub> = 0.5333 → WVTR = <strong>1.87 g/m²·day</strong></em>
</div>
<h3 style="font-size:1.1rem; color:var(--text); margin:1.2rem 0 0.5rem 0; font-family:sans-serif;">Hygroscopic dynamics</h3>
<div style="background:#f8fafc; padding:1.1rem; border-radius:6px; font-family:monospace; font-size:0.95rem; text-align:center; border:1px dashed var(--border); margin:1rem 0; color:#0f172a;">
Corrected Permeability = Permeability<sub>base</sub> × e<sup>β × (ΔRH)</sup>
</div>
...
function renderCalc() {
  ...
  var cardHeader = '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.75rem">' +
    ...
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:16px;height:16px;color:var(--primary)"><rect x="4" y="2" width="16" height="20" rx="2"/><line x1="8" y1="6" x2="16" y2="6"/></svg>' +
    'Laminate Structure</h2>' +
  ...
  var testMethodFilter =
    ...
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:14px;height:14px"><path d="M22 19a2 2 0 01-2 2H4a2 2 0 01-2-2V5a2 2 0 012-2h5l2 3h9a2 2 0 012 2z"/><line x1="12" y1="11" x2="12" y2="17"/><line x1="9" y1="14" x2="15" y2="14"/></svg>' +
    'Filter by Test Method</label>' +
  ...
  if (!hasMats) {
    ...
    for (var ci2 = 0; ci2 < uniqueConditions.length; ci2++) {
      var c2 = uniqueConditions[ci2];
      var selOpt = State.selCond && State.selCond.temperature === c2.temperature && State.selCond.humidity === c2.humidity ? ' selected' : '';
      opts += '<option value="' + c2.temperature + '|' + c2.humidity + '"' + selOpt + '>' + c2.temperature + '\u00b0C / ' + c2.humidity + '%</option>';
    }
  ...
  for (var i = 0; i < State.layers.length; i++) {
    ...
    var mat = null;
    if (l.mid !== null) {
      for (var mm = 0; mm < DB.materials.length; mm++) {
        if (DB.materials[mm].id === l.mid) { mat = DB.materials[mm]; break; }
      }
    }
    ...
    for (var j = 0; j < displayMats.length; j++) {
      var dm = displayMats[j];
      var label = dm.name;
      var currentTM2 = State.mode === 'wvtr' ? (dm.testMethodWVTR || '') : (dm.testMethodOTR || '');
      if (currentTM2) label += ' [' + currentTM2 + ']';
      var selected = (l.mid === dm.id) ? ' selected' : '';
      matOpts += '<option value="' + dm.id + '"' + selected + '>' + label + '</option>';
    }
  ...
  if (hasResult && State.calcResult.layers) {
    var hygroWarnings = [];
    for (var rr = 0; rr < State.calcResult.layers.length; rr++) {
      var lr = State.calcResult.layers[rr];
    ...
    if (hygroWarnings.length > 0) {
      ...
      resultHTML += `<div class="alert alert-warning" style="margin-top:0.75rem; display:flex; gap:0.4rem; align-items:flex-start; font-size:0.75rem">
        ...
        <div style="flex:1">
          ...
          ${warningsHTML}
          <div style="margin-top:0.4rem; font-size:0.68rem; color:var(--text-light); border-top:1px solid rgba(0,0,0,0.1); padding-top:0.3rem">β correction based on external RH vs material reference conditions</div>
        </div>
      </div>`;
    }
  }
...
<div style="padding:1.2rem 1.5rem;">
<h2 style="font-family:Georgia, 'Times New Roman', serif; font-size:1.2rem; color:var(--text); border-bottom:1px solid var(--border); padding-bottom:0.5rem; margin-bottom:1rem;">Understanding Arrhenius Analysis</h2>
...
<strong>Think of it this way:</strong> Imagine trying to walk through a crowded room. When it's cool, people move slowly and you make progress gradually. When it's warm and energetic, everyone's moving faster—and so do the molecules trying to pass through your packaging.
</div>
<h3 style="font-family:-apple-system, BlinkMacSystemFont, sans-serif; font-size:1.05rem; color:var(--primary-dark); margin:1.2rem 0 0.5rem 0; font-weight:700;">The Mathematical Equation</h3>
...
<h3 style="font-family:-apple-system, BlinkMacSystemFont, sans-serif; font-size:1.05rem; color:var(--primary-dark); margin:1.2rem 0 0.5rem 0; font-weight:700;">Reading your R² score</h3>
<div style="display:grid; grid-template-columns:1fr; gap:0.5rem; margin:0.8rem 0;">
<div style="background:var(--success-light); padding:0.6rem; border-radius:6px; border-left:3px solid var(--success);"><strong style="color:var(--success);">✓ R² ≥ 0.95:</strong> Excellent fit. Predictions are highly reliable.</div>
<div style="background:var(--warning-light); padding:0.6rem; border-radius:6px; border-left:3px solid var(--warning);"><strong style="color:var(--warning);">⚠ 0.80 ≤ R² &lt; 0.95:</strong> Reasonable fit. Use caution for extrapolation.</div>
<div style="background:var(--danger-light); padding:0.6rem; border-radius:6px; border-left:3px solid var(--danger);"><strong style="color:var(--danger);">✗ R² &lt; 0.80:</strong> Weak fit. Collect more data points.</div>
</div>
<div style="margin-top:1.5rem; padding:0.9rem; background:var(--bg); border-radius:8px; font-size:0.88rem; color:var(--text-light); border-left:4px solid var(--primary); font-family:sans-serif;">
<strong>Operational note:</strong> For commercial shelf-life claims or regulatory submissions, always validate predictions with real-time or accelerated aging studies per ASTM F1249 or ISO 15106.
</div>
</div>
</div>
</div>`;
...
function renderArrhenius() {
  ...
  return '<div class="card"><h2>Arrhenius Analysis</h2>' +
    '<p style="font-size:.78rem;color:var(--text-light);margin-bottom:.75rem">Predict ' + getLabel() + ' at unmeasured temperatures using the Arrhenius equation. ' +
    '<span style="color:' + (validCount > 0 ? 'var(--success)' : 'var(--danger)') + ';font-weight:600">' + infoText + '</span></p>' +
  ...
  setTimeout(function() {
    ...
    if (!sel.value) {
      if (resEl) resEl.innerHTML =
        ...
        '<span style="font-size:0.75rem;color:var(--text-light)">Choose a material with ✓ Arrhenius badge (multi-temperature data required)</span></div></div>';
...
function onArrChange() {
  ...
  var mat = null;
  for (var i = 0; i < DB.materials.length; i++) {
    if (DB.materials[i].id === matId) { mat = DB.materials[i]; break; }
  }
  if (!mat) return;
  ...
  if (isNaN(customEa) || customEa <= 0) {
    ...
    var R2 = Engine.R_GAS;
    var lnA_values = [];
    for (var i2 = 0; i2 < mat.validConditions.length; i2++) {
      var tk = mat.validConditions[i2].temperature + 273.15;
      if (vals2[i2] && vals2[i2].value > 0) lnA_values.push(Math.log(vals2[i2].value) + EaUsed / (R2 * tk));
    }
    if (lnA_values.length > 0) {
      var lnA_mean = 0;
      for (var j2 = 0; j2 < lnA_values.length; j2++) lnA_mean += lnA_values[j2];
      lnA_mean /= lnA_values.length;
      A = Math.exp(lnA_mean);
  ...
  for (var i3 = 0; i3 < mat.validConditions.length; i3++) {
    var t3 = mat.validConditions[i3].temperature;
    if (t3 < minT) minT = t3; if (t3 > maxT) maxT = t3;
  }
  ...
  if (resEl) {
    resEl.innerHTML = warn +
      '<div class="reliability-meter ' + relClass + '">' +
      (isNaN(customEaInput) || customEaInput === '' ? 'R²=' + (rSquared).toFixed(4) + ' ' : '') +
      'Eₐ=' + (EaUsed / 1000).toFixed(2) + ' kJ/mol</div>' +
  ...
  var dataPoints = [];
  var valsArr = Engine.getValues(mat);
  for (var i4 = 0; i4 < mat.validConditions.length; i4++) {
    if (valsArr[i4] && valsArr[i4].value > 0 && mat.validConditions[i4]) {
      dataPoints.push({ T_K: mat.validConditions[i4].temperature + 273.15, trans: valsArr[i4].value });
    }
  }
...
function clearArrCanvas(id) {
  ...
  var width = c.clientWidth || 500;
  var height = c.clientHeight || 280;
  c.width = width * dpr; c.height = height * dpr;
  c.style.width = width + 'px'; c.style.height = height + 'px';
  ctx.scale(dpr, dpr);
  ctx.fillStyle = '#f8fafc'; ctx.fillRect(0, 0, width, height);
  ctx.fillStyle = '#94a3b8'; ctx.font = window.innerWidth < 480 ? '11px sans-serif' : '13px sans-serif';
  ctx.textAlign = 'center'; ctx.fillText('Select a multi-temp material', width / 2, height / 2);
}
...
<div style="background:var(--primary-light); padding:0.8rem 1rem; border-radius:8px; border-left:3px solid var(--primary); margin:1rem 0; font-family:sans-serif; font-size:0.9rem;">
<strong>The Mathematical Phenomenon:</strong> Even though a single layer's resistance scales linearly with its thickness, the final transmission rate changes non-linearly because the variable layer constantly shifts its percentage share of the global resistance pool.
</div>
<div style="background:#f8fafc; padding:1.1rem; border-radius:6px; font-family:monospace; font-size:0.95rem; text-align:center; border:1px dashed var(--border); margin:1rem 0; color:#0f172a;">
R<sub>variable</sub>(t) = Thickness<sub>sampled</sub> / (Permeability<sub>ref</sub> × Thickness<sub>ref</sub>)<br><br>
Laminate Permeability(t) = 1 / (R<sub>fixed</sub> + R<sub>variable</sub>(t))
</div>
<div style="margin-top:1.5rem; padding:0.9rem; background:var(--bg); border-radius:8px; font-size:0.88rem; color:var(--text-light); border-left:4px solid var(--primary); font-family:sans-serif;">
...
function doOptimize() {
  ...
  if (!condition || !State.layers.every(function(l) { return l.mid !== null && l.thick > 0; })) { resEl.innerHTML = '<div class="alert alert-info">Configure and calculate first</div>'; return; }
  ...
  var mat = null; for (var m = 0; m < DB.materials.length; m++) if (DB.materials[m].id === State.layers[barrierIdx].mid) { mat = DB.materials[m]; break; }
...
function postCompareRender() {
  ...
  if (tableEl && selected.length > 0) {
    ...
    var th = '<thead><tr><th>Parameter</th>'; for (var i2 = 0; i2 < selected.length; i2++) th += '<th>' + selected[i2].name + '</th>'; th += '</tr></thead>';
    var body = '<tbody>';
    body += '<tr><td><strong>' + modeLabel + '</strong></td>'; for (var i3 = 0; i3 < selected.length; i3++) body += '<td>' + selected[i3].total.toFixed(4) + ' ' + unit + '</td>'; body += '</tr>';
    body += '<tr><td><strong>Thickness</strong></td>'; for (var i4 = 0; i4 < selected.length; i4++) body += '<td>' + selected[i4].totalThickness.toFixed(0) + ' um</td>'; body += '</tr>';
    body += '<tr><td><strong>Conditions</strong></td>'; for (var i5 = 0; i5 < selected.length; i5++) body += '<td>' + selected[i5].temperature + '\u00b0C / ' + selected[i5].humidity + '%</td>'; body += '</tr>';
    body += '<tr><td><strong>Recyclable</strong></td>'; for (var i6 = 0; i6 < selected.length; i6++) body += '<td><span class="sustainability-flag ' + (selected[i6].recyclable ? 'yes' : 'no') + '">' + (selected[i6].recyclable ? 'Yes' : 'No') + '</span></td>'; body += '</tr>';
    body += '</tbody>';
  ...
  if (selected.length >= 2) {
    ...
    for (var i7 = 0; i7 < selected.length; i7++) { labels.push(selected[i7].name); vals.push(selected[i7].total); colors.push(LAYER_COLORS[i7 % LAYER_COLORS.length]); }
...
function drawLamChart() {
  ...
  var ctx = canvas.getContext('2d');
  var labels = [], vals = [], colors = [];
  for (var i = 0; i < DB.laminates.length; i++) { labels.push(DB.laminates[i].name); vals.push(DB.laminates[i].total); colors.push(LAYER_COLORS[i % LAYER_COLORS.length]); }
  chartInstances.lam = new Chart(ctx, { type: 'bar', data: { labels: labels, datasets: [{ label: (DB.laminates[0].mode || State.mode).toUpperCase(), data: vals, backgroundColor: colors, borderRadius: 6 }] }, options: { responsive: true, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, title: { display: true, text: getUnit() } }, x: { title: { display: true, text: 'Laminates' } } } } });
}
